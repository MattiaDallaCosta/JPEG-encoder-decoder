...
void rgb_to_dct_block(uint8_t *in, int16_t *Y, int16_t *Cb, int16_t *Cr,int offx, int offy, area_t dims) {
  ...
  int index = 3*((offy*16+dims.y+l)*WIDTH+offx*16+dims.x+r);        // index of red component of pixel in input array
	midY[i]            =       0.299    * in[index] + 0.587    * in[index+1] + 0.114    * in[index+2]; // Rgb -> Y
	app[0][(l%2)*16+r] = 128 - 0.168736 * in[index] - 0.331264 * in[index+1] + 0.5      * in[index+2]; // Rgb -> Cb
	app[1][(l%2)*16+r] = 128 + 0.5      * in[index] - 0.418688 * in[index+1] - 0.081312 * in[index+2]; // Rgb -> Cr
    if (r%2 == 1 && l%2 == 1) {
...
size_t write_file(char * filename, int16_t Y[PIX_LEN], int16_t Cb[PIX_LEN/4], int16_t Cr[PIX_LEN/4], area_t dims, huff_code Luma[2], huff_code Chroma[2]) {
	...
	fputc(0xFF, f); fputc(0xDA, f); // SOS Symbol
		...
		write_coefficients(f, dims.w*dims.h, Y, &Luma[0], &Luma[1]);
		fill_last_byte(f);
	...
	fputc(0xFF, f); fputc(0xDA, f); // SOS Symbol
		...
		fputc(0x3F, f); // end of spectral selection - default value
		fputc(0x00, f); // successive approximation bits - default value
		write_coefficients(f, dims.w*dims.h/4, Cb, &Chroma[0], &Chroma[1]);
		fill_last_byte(f);
	...
	fputc(0xFF, f); fputc(0xDA, f); // SOS Symbol
		...
		fputc(0x3F, f); // end of spectral selection - default value
		fputc(0x00, f); // successive approximation bits - default value
		write_coefficients(f, dims.w*dims.h/4, Cr, &Chroma[0], &Chroma[1]);
		fill_last_byte(f);
