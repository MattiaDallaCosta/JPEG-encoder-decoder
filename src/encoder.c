...
void rgb_to_dct(uint8_t *in, int16_t Y[PIX_LEN], int16_t CbCr[2][PIX_LEN/4], area_t dims) {
  ...
  int begin, j;
    ...
    if (r%8 == 7 && l%8 == 7) {
      ...
      if (ih%2 == 1 && iv%2 == 1) {
        ...
        dct_block(dims.w/2, mid[1] + (iv*dims.w + ih*2)*2, CbCr[0] + (iv*dims.w/16+ih)*32, chroma_quantizer);
        dct_block(dims.w/2, mid[2] + (iv*dims.w + ih*2)*2, CbCr[1] + (iv*dims.w/16+ih)*32, chroma_quantizer);
        CbCr[0][(iv*dims.w/16+ih)*32] -= last[1];
        last[1] += CbCr[0][(iv*dims.w/16+ih)*32];
        CbCr[1][(iv*dims.w/16+ih)*32] -= last[2];
        last[2] += CbCr[1][(iv*dims.w/16+ih)*32];
      }
    }
    if (r == (dims.w - 1)){
      printf("off : %i\n", off);
