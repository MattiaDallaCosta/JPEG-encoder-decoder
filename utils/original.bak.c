...
void init_dct_lookup()
{
	...
	for (i=0; i<8; i++)
		for (j=0; j<8; j++)
		{
			cos_lookup[i][j] = cos((double)((2*i+1)*j*M_PI/16));
 ...
 */
void dct(int blocks_horiz, int blocks_vert, int in[], double out[], int type)
{
  FILE * f0 = fopen(type == 0 ? "hisParts/Y-Dct" : (type == 1 ? "hisParts/Cb-Dct" : "hisParts/Cr-Dct"), "w");
	int h,v;
	for (v=0; v<blocks_vert; v++)
		for (h=0; h<blocks_horiz; h++) {
			dct_block(8*blocks_horiz, in + v*blocks_horiz*64 + h*8, out + (v*blocks_horiz+h)*64);
      int i = (v*blocks_horiz+h);
      for (int k = 0; k < 64; k++) {
        fprintf(f0," %f%s", *(out + i*64 + k), k%8 == 7 ? "\n" : " ");
      }
      fprintf(f0," ---====--- \n");
    }
    fclose(f0);
...
void write_file(char* file_name, jpeg_data* data)
{
	...
	fputc(0xFF, f); fputc(0xDB, f); // DQT Symbol
		...
		int i;
		for (i=0; i<64; i++) fputc(luma_quantizer[scan_order[i]], f);

	fputc(0xFF, f); fputc(0xDB, f); // DQT Symbol
		fputc(0, f); fputc(67, f); // len
		fputc(1, f); // quant-table id
		for (i=0; i<64; i++) fputc(chroma_quantizer[scan_order[i]], f);
...
int main(int argc, char** args)
{
	...
	fflush(stdout);
	init_dct_lookup();
	dct(data.width/8, data.height/8, data.y, data.dct_y, 0);
	dct(data.width/16, data.height/16, data.cb_sub, data.dct_cb, 1);
	dct(data.width/16, data.height/16, data.cr_sub, data.dct_cr, 2);
	printf("%10.3f ms\n", timer());
	...
	printf("%10.3f ms\n", timer());
  FILE * f0 = fopen("hisParts/Y-Quant", "w");
  FILE * f1 = fopen("hisParts/Cb-Quant", "w");
  FILE * f2 = fopen("hisParts/Cr-Quant", "w");
  for (int i = 0; i < data.num_pixel/64; i++) {
    for (int k = 0; k < 64; k++) {
      fprintf(f0," %i%s", data.dct_y_quant[i*64 + k], k%8 == 7 ? "\n" : " ");
    }
    fprintf(f0," ---====--- \n");
    if (!(i%4)) {
      for (int k = 0; k < 64; k++) {
        fprintf(f1," %i%s", data.dct_cb_quant[i*64 + k], k%8 == 7 ? "\n" : " ");
        fprintf(f2," %i%s", data.dct_cr_quant[i*64 + k], k%8 == 7 ? "\n" : " ");
      }
      fprintf(f1," ---====--- \n");
      fprintf(f2," ---====--- \n");
    }
  ...
  for (int i = 0; i < data.height*data.width; i++) {
    fprintf(f0," %i%s",data.dct_y_quant[i], i%data.width == 0 ? "\n" : " ");
  }
  for (int i = 0; i < data.height*data.width/4; i++) {
    fprintf(f1," %i%s", data.dct_cb_quant[i], i%(data.width/2) == 0 ? "\n" : " ");
    fprintf(f2," %i%s", data.dct_cr_quant[i], i%(data.width/2) == 0 ? "\n" : " ");
  }
  fclose(f0);
  fclose(f1);
  fclose(f2);
