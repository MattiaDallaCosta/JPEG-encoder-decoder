...
int main(int argc, char** args)
{
  ...
  FILE * f0 = fopen( "hisParts/Y-Pre", "w");
  FILE * f1 = fopen("hisParts/Cb-Pre", "w");
  FILE * f2 = fopen("hisParts/Cr-Pre", "w");
  for (int i = 0; i < data.num_pixel; i++) {
    fprintf(f0,"%i%s", data.y[i], i%data.width == data.width-1 ? "\n" : " ");
    if ((i%data.width)%2 == 0 && (i/data.width)%2 == 0) {
      int h = (i/data.width)/2*(data.width/2);
      int w = (i%data.width)/2;
      fprintf(f1,"%i%s", data.cb_sub[h+w], (h+w)%(data.width/2) == (data.width/2)-1 ? "\n" : " ");
      fprintf(f2,"%i%s", data.cr_sub[h+w], (h+w)%(data.width/2) == (data.width/2)-1 ? "\n" : " ");
    }
  }
  fclose(f0);
  fclose(f1);
  ...
  f0 = fopen("hisParts/Y-Dct", "w");
  f1 = fopen("hisParts/Cb-Dct", "w");
  f2 = fopen("hisParts/Cr-Dct", "w");
  for (int i = 0; i < data.num_pixel; i++) {
    fprintf(f0,"%f%s", data.dct_y[i], i%data.width == data.width-1 ? "\n" : " ");
    if ((i%data.width)%2 == 0 && (i/data.width)%2 == 0) {
      int h = (i/data.width)/2*(data.width/2);
      int w = (i%data.width)/2;
      fprintf(f1,"%f%s", data.dct_cb[h+w], (h+w)%(data.width/2) == (data.width/2)-1 ? "\n" : " ");
      fprintf(f2,"%f%s", data.dct_cr[h+w], (h+w)%(data.width/2) == (data.width/2)-1 ? "\n" : " ");
    }
  }
  fclose(f0);
  fclose(f1);
  ...
  f0 = fopen("hisParts/Y-Quant", "w");
  f1 = fopen("hisParts/Cb-Quant", "w");
  f2 = fopen("hisParts/Cr-Quant", "w");
  for (int i = 0; i < data.num_pixel; i++) {
    fprintf(f0,"%i%s", data.dct_y_quant[i], i%data.width == data.width-1 ? "\n" : " ");
    if (!(i%4)) {
      fprintf(f1,"%i%s", data.dct_cb_quant[i], (i/4)%(data.width/2) == (data.width/2)-1 ? "\n" : " ");
      fprintf(f2,"%i%s", data.dct_cr_quant[i], (i/4)%(data.width/2) == (data.width/2)-1 ? "\n" : " ");
    }
  }
  fclose(f0);
  fclose(f1);
  ...
  f0 = fopen("hisParts/Y-ZigZag", "w");
  f1 = fopen("hisParts/Cb-ZigZag", "w");
  f2 = fopen("hisParts/Cr-ZigZag", "w");
  for (int i = 0; i < data.num_pixel; i++) {
    fprintf(f0,"%i%s", data.dct_y_quant[i], i%data.width == data.width-1 ? "\n" : " ");
    if (!(i%4)) {
      fprintf(f1,"%i%s", data.dct_cb_quant[i], (i/4)%(data.width/2) == (data.width/2)-1 ? "\n" : " ");
  ...
  f0 = fopen("hisParts/Y-Diff", "w");
  f1 = fopen("hisParts/Cb-Diff", "w");
  f2 = fopen("hisParts/Cr-Diff", "w");
  for (int i = 0; i < data.num_pixel; i++) {
    fprintf(f0,"%i%s", data.dct_y_quant[i], i%data.width == data.width-1 ? "\n" : " ");
    if (!(i%4)) {
      fprintf(f1,"%i%s", data.dct_cb_quant[i], (i/4)%(data.width/2) == (data.width/2)-1 ? "\n" : " ");
